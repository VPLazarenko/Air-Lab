...
export default function Dashboard() {
  ...
  const handleIntegrationClick = (integration: string) => {
    ...
    console.log(`Настройка интеграции: ${integration}`);
    alert(`Настройка интеграции с ${integration} будет реализована позже`);
  };
            ...
            >
              ...
              <CardContent>
                <div className="text-lg font-bold text-purple-600">
                  {user?.apiKey ? 'Настроено' : 'Настроить'}
                </div>
                <p className="text-xs text-gray-600 dark:text-gray-400">
                  Подключение к OpenAI Assistants API
                </p>
                <Badge variant={user?.apiKey ? "default" : "outline"} className="mt-2">
                  {user?.apiKey ? 'Подключено' : 'Не настроено'}
                </Badge>
              </CardContent>
            </Card>
          </div>
